...
/* pnm.c - gcc -Wall -O2 -c pnm_list.o pnm.c -o pnm.o */
...
#define pnm_in_range(X, INF, SUP) ((X) < (INF) ? PNM_FALSE : \
                                   (X) > (SUP) ? PNM_FALSE : \
                                   PNM_TRUE)

double pnm_fmod (double x, int q) {
    while (x < 0) x += q;
    while (x > q) x -= q;
    return x;
}
...
void pnm_destroy (pnm_context_t *ctx) {
    while (ctx->comments->first != NULL) {
        pnm_list_delete_first(ctx->comments);
    }
...
void pnm_replace_comment (pnm_context_t *ctx, int pos, const char *comment) {
    if (pos >= ctx->__comm_q) {
...
void pnm_delete_comment (pnm_context_t *ctx, int pos) {
    if (pos >= ctx->__comm_q) {
...
pnm_pixel_t pnm_pixel_get (pnm_context_t *ctx,
                           ...
                           unsigned int y)
{
    if (pnm_in_range(x, 0, ctx->width - 1)) {
        fprintf(stderr,
    ...
    if (pnm_in_range(y, 0, ctx->height - 1)) {
        fprintf(stderr,
                    ...
                    pnm_pixel_t color)
{
    ...
    if (pnm_in_range(color.r, 0, ctx->maxval)) {
        fprintf(stderr,
                "`r' color component is out of range "
                "(must be between 0 and %u and current value is `%u').\n",
                ctx->maxval, color.r);
        pnm_error_exit(ctx, PNMERR_VALUE_OUT_OF_RANGE);
    } else if (pnm_in_range(color.g, 0, ctx->maxval)) {
        fprintf(stderr,
                "`g' color component is out of range "
                "(must be between 0 and %u and current value is `%u').\n",
                ctx->maxval, color.g);
        pnm_error_exit(ctx, PNMERR_VALUE_OUT_OF_RANGE);
    } else if (pnm_in_range(color.b, 0, ctx->maxval)) {
        fprintf(stderr,
                "`b' color component is out of range "
                "(must be between 0 and %u and current value is `%u').\n",
                ctx->maxval, color.b);
        pnm_error_exit(ctx, PNMERR_VALUE_OUT_OF_RANGE);
    }
...
void pnm_set_color (pnm_context_t *ctx, pnm_pixel_t p) {
   ...
   while (i < ctx->width) {
    ...
    i = 0;
    while (i < ctx->height) {
        memmove(ctx->canvas + i * ctx->height,
                ctx->canvas,
                sizeof(pnm_pixel_t) * ctx->width);
        i++;
    }
}
...
void pnm_read_ascii (pnm_context_t *ctx, FILE *fp) {
...
void pnm_read_bin (pnm_context_t *ctx, FILE *fp) {
...
pnm_bool_t pnm_write_bin (pnm_context_t *ctx, const char *filename) {
    FILE *fp;
    int j, bw, t;
...
pnm_bool_t pnm_write_ascii (pnm_context_t *ctx, const char *filename) {
    FILE *fp;
    pnm_pixel_t p;
    int j;
